...
module.exports = async () => {
    const commands = [];
    const commandFiles = readdirSync(path.resolve(process.cwd(), './libs/interactions')).filter(file => file.endsWith('.js'));

    for (const file of commandFiles) {
        logger.info(`Loading interaction ./libs/interactions/${file}`)
